...
public enum ParserMessage {
	...
	public String formatMessage(boolean useCode, int position, Object... inserts) {
		...
		if (useCode) {
			msg.append(code);
			if (type == org.springframework.shell.command.parser.ParserMessage$Type.WARNING) {
				msg.append("W");
			} else if (type == org.springframework.shell.command.parser.ParserMessage$Type.ERROR) {
				msg.append("E");
			}
			msg.append(":");
		}
